...
export function HostSidebar() {
  ...
  const displayName = user?.fullName || user?.email || 'Host Account';
  ...
  return (
    <aside className="w-64 bg-white border-r border-gray-200 min-h-[calc(100vh-64px)] p-4 flex flex-col justify-between shrink-0">
      ...
      {/* Bottom Account & Logout Section for Host UX */}
      <div className="pt-4 border-t border-gray-100 space-y-3">
        <div className="flex items-center gap-3 px-2">
          ...
          <div className="truncate text-left">
            ...
            <p className="text-[10px] text-gray-500 font-semibold truncate">{user?.email || 'host@bookong.vn'}</p>
          </div>
        </div>
